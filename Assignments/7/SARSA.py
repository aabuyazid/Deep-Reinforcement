...
class Sarsa(AbstractSolver):
    ...
    def train_episode(self):
        ...
        ################################
        alpha, gamma = self.options.alpha, self.options.gamma
        done = False
        while not done:
            prob_actions = self.epsilon_greedy_action(state)
            best_actions = np.flatnonzero(prob_actions == prob_actions.max())
            action = np.random.choice(best_actions)

            next_state, reward, done, _ = self.step(action)

            prob_actions = self.epsilon_greedy_action(next_state)
            best_actions = np.flatnonzero(prob_actions == prob_actions.max())
