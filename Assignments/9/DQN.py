...
class DQN(AbstractSolver):
    ...
    def epsilon_greedy(self, state):
        ...
        q_tensor = self.model(state_tensor)
        q = q_tensor.detach().numpy()
    ...
    def compute_target_values(self, next_states, rewards, dones):
        ...
        The parameters are torch tensors!
        """
        ################################
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        gamma = torch.tensor(
                self.options.gamma, 
                dtype=torch.float32, 
                device=next_states.device  # Ensure gamma is on the same device
            )
        with torch.no_grad():  # Explicitly disable gradients (best practice)
            next_q_values = self.target_model(next_states)
            max_next_q = torch.max(next_q_values, dim=-1).values
        
        # Mask out Q-values for terminal states (optional but efficient)
        max_next_q = max_next_q * (1.0 - dones)
            
        target_q = rewards + gamma * max_next_q
        return torch.where(dones > 0.0, rewards, target_q)
    ...
    def train_episode(self):
        ...
        N = self.options.update_target_estimator_every
